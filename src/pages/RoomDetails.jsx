...
function RoomDetails() {
  ...
  const room = SAMPLE_ROOMS.find((item) => String(item.id) === String(id));
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log({ room: room.title, ...formData });
    setSubmitted(true);
  };
    ...
    <section className="room-details-page">
      ...
      <div className="room-details-banner">
        <div className="container">
          <button className="room-details-back" onClick={() => navigate("/")}>
            <i className="bi bi-arrow-left"></i>
            Back to Rooms
          </button>

          <div className="room-details-breadcrumb">
            HOME <span>/</span> ROOMS <span>/</span> {room.title.toUpperCase()}
          </div>
        ...
        <div className="row g-5">
          ...
          <div className="col-lg-8">
            <div className="room-details-image-wrapper">
              <img src={room.image} alt={room.title} className="room-details-main-image" />
            ...
            <div className="room-details-stats">
              <div className="room-detail-stat">
                ...
                <div><strong>{room.rooms}</strong><span>Room</span></div>
              </div>
              <div className="room-detail-stat">
                <i className="bi bi-droplet-fill"></i>
                <div><strong>{room.bathrooms}</strong><span>Bathroom</span></div>
              </div>
              <div className="room-detail-stat">
                <i className="bi bi-people-fill"></i>
                <div><strong>{room.guests}</strong><span>Guests</span></div>
              </div>
              <div className="room-detail-stat">
                <i className="bi bi-bounding-box"></i>
                <div><strong>{room.area}</strong><span>Room Size</span></div>
              </div>
            </div>

            <div className="room-details-content">
              <div className="room-details-section-title">
                <span></span><p>ABOUT THE ROOM</p><span></span>
              </div>
              <h2>Experience Comfort & Luxury</h2>
              <p className="room-details-description">
                Welcome to {room.title}, a thoughtfully designed space created for comfort,
            ...
            <div className="room-details-amenities">
              <div className="room-details-section-title">
                <span></span><p>ROOM FEATURES</p><span></span>
              </div>
              <h2>Premium Amenities</h2>
              <div className="room-details-amenities-grid">
                {room.amenities?.map((amenity, index) => (
                  <div className="room-details-amenity" key={index}>
          ...
          <div className="col-lg-4">
            <div className="room-enquiry-card">
              <div className="room-enquiry-top">
                <p>STARTING FROM</p>
                <h2>₹{room.price.toLocaleString("en-IN")}</h2>
                <span>Per Night</span>
              </div>
              ...
              {!submitted ? (
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="room-form-group">
                    ...
                    <select name="guests" value={formData.guests} onChange={handleChange}>
                      {[...Array(room.guests)].map((_, index) => (
                        <option key={index} value={index + 1}>{index + 1} Guest</option>
                      ))}
                    </select>
                  </div>
                  ...
                  <div className="room-form-group">
                    <label>Message</label>
                    <textarea name="message" placeholder="Any special requests?" value={formData.message} onChange={handleChange} rows="4" />
                  </div>

                  <button type="submit" className="room-enquiry-btn">
                    Send Enquiry
