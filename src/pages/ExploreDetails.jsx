...
function ExploreDetail() {
  ...
  const article = rawArticle
    ? { ...rawArticle, image: IMAGE_MAP[rawArticle.image] || rawArticle.image }
    : null;

  const currentIndex = exploreData.findIndex((item) => item.id === id);
  const nextRaw = exploreData[(currentIndex + 1) % exploreData.length];
  const nextArticle = { ...nextRaw, image: IMAGE_MAP[nextRaw.image] || nextRaw.image };
  ...
  return (
    <div className="explore-detail">
      {/* HERO — image + title only, no overlapping text */}
      ...
      <div className="explore-detail-body section">
        ...
        <div className="explore-detail-next">
          <span>Next up</span>
          <Link to={`/explore/${nextArticle.id}`} className="explore-detail-next-link">
